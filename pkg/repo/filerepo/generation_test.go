...
func TestGenerationManagerConditionalPollAdvancesFreshness(t *testing.T) {
	...
	require.NoError(t, err)
	before := handler.current["repo"].validatedAt
	time.Sleep(time.Millisecond)
	_, err = handler.Refresh(context.Background(), 1)
	require.NoError(t, err)
	require.Equal(t, `"v1"`, validators.Get("If-None-Match"))
	require.True(t, handler.current["repo"].validatedAt.After(before))
	require.Equal(t, committed.PublishedAt, handler.Current("repo").PublishedAt)
	require.Equal(t, `"v2"`, handler.current["repo"].header.Get("ETag"))
	...
	require.NotEqual(t, markerBefore, markerAfter)
...
func TestGenerationManagerUnchangedPollAdvancesFreshnessWithoutCandidate(t *testing.T) {
	...
	require.Equal(t, committed.PublishedAt, current.PublishedAt)
	require.True(t, handler.current["repo"].validatedAt.After(committed.PublishedAt))
	require.Equal(t, `"v1"`, validators.Get("If-None-Match"))
	require.Equal(t, `"v2"`, handler.current["repo"].header.Get("ETag"))
	handler.mu.RLock()
...
func TestGenerationManagerSchedulerPublishesChangedAnchor(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, status)
	require.Equal(t, "HIT", cache)
	require.Equal(t, anchors[1], response.Body.Bytes())
	require.Eventually(t, func() bool {
		current := manager.Current("repo")
...
func TestGenerationManagerPendingRetryAllowsOtherRoots(t *testing.T) {
	...
	handler.requestCurrentPoll("b", false)
	_, err = handler.refresh(context.Background(), 2)
	require.ErrorIs(t, err, io.ErrUnexpectedEOF)
	more, err := handler.refresh(context.Background(), 1)
	require.NoError(t, err)
	require.False(t, more)
	...
	require.NoError(t, err)
	require.False(t, more)
	require.Equal(t, int32(2), currentPolls.Load())
}
