...
func FuzzOCIRequestParsers(f *testing.F) {
	...
	f.Add(uint8(1), "library/alpine:latest")
	f.Add(uint8(2), "/v2/library/alpine/manifests/latest")
	f.Add(uint8(2), "/v2/library/alpine/blobs/sha256:"+strings.Repeat("a", 64))
	...
	f.Fuzz(func(t *testing.T, kind uint8, input string) {
		...
		case 1:
			result, err := parseOCIRef(input)
			if err == nil {
				require.True(t, strings.HasPrefix(result, "v2/"))
			}
		case 2:
