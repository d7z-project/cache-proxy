...
func (Driver) Validate(spec *proxydriver.ResolvedSpec) error {
	...
	if !validPolicy(policy.DefaultPolicy) {
		return fmt.Errorf("invalid file default policy %q", policy.DefaultPolicy)
	}
	if policy.BusyPolicy != config.BusyPolicyBypass && policy.BusyPolicy != config.BusyPolicyStale {
		return fmt.Errorf("invalid file busy policy %q", policy.BusyPolicy)
	}
	...
	for i, rule := range policy.Rules {
		...
		if !validPolicy(rule.Policy) {
			return fmt.Errorf("file rule %d: invalid policy %q", i, rule.Policy)
		}
		policy.Rules[i] = rule
	}
	return nil
