...
func (Driver) Plan(_ context.Context, plan *proxyruntime.InstancePlan) error {
	...
	handler := filerepo.NewHandler(plan.Name(), config.ModeRPM, config.ModeRPM, config.Freshness(time.Minute), classify, block.Upstreams, block.Transport, expireAfter, &block.Policy, plan.Store(), plan.Stats())
