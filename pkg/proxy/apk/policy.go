...
func (Driver) Plan(_ context.Context, plan *proxyruntime.InstancePlan) error {
	...
	handler := filerepo.NewIndexedHandler(plan.Name(), config.ModeAPK, config.ModeAPK, config.Freshness(time.Minute), classify, upstreams, block.Transport, expireAfter, &block.Policy, filerepo.RefreshPolicy{
		Interval: time.Hour,
