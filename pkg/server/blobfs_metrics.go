...
func (c *blobfsCollector) Collect(ch chan<- prometheus.Metric) {
	...
	ch <- prometheus.MustNewConstMetric(c.chunks, prometheus.GaugeValue, float64(stats.Chunks.Corrupt), "corrupt")
	...
	ch <- prometheus.MustNewConstMetric(c.segments, prometheus.GaugeValue, float64(stats.Segments.Corrupt), "corrupt")
