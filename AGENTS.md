...
- helper 应有实际复用或明确职责边界；内联短小单次转发，合并重复生命周期逻辑，不为拆文件新增接口层。按职责判断拆分，不能只按调用次数内联解析、校验或资源管理代码。
...
- freshness、内容创建时间与 retention 分开处理；条件验证绑定原 upstream。响应复用遵守更短的上游缓存策略、协议有效期及签名期限；遵守 `no-store`。
- 仓库后台轮询使用配置周期和稳定正向偏移，与响应 freshness 独立；请求验证、首次填充和缺失恢复可提前触发并按 root 合并，事件必须区分实际工作与空检查。
...
- scheduler 单 goroutine 串行执行，刷新平滑分布、失败有界退避；失败 root 退避期间仍处理其他就绪 root。请求热路径只触发内存状态，不做调度状态 fsync。
- GC 按检查对象数分批并用有界游标继续；保护 current、精确 previous、pending、active reader 和 grace-period 对象，删除前复查引用。清理仅作用于拥有的数据，过期临时文件和闲置动态状态应可回收。

## 测试与验收

- 行为变化必须补测试；测试只断言当前有效协议、接口与资源约束，并与实现同步维护。
